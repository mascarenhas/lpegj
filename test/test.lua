...
t = {m.match({"S", S = m.C(m.C(1) * m.V("S") + -1)}, "abc")}
...
for n = 1, 550 do
  ...
  assert(n < 13 or m.match(m.Cc(20) * (m.P(n - 13) * m.P(10)) * 3, x) == 20)
  local n3 = m.P(math.floor(n/3))
  assert(m.match(n3 * m.Cp() * n3 * n3, x) == math.floor(n/3) + 1)
end
...
assert(m.match((m.P(3) +  m.P(4) * m.Cp()) * "a", "abca") == 5)
...
p = { "S", S = ('a' * m.V("S"))^-1 } * m.P'b' * { "S", S = 'a' * m.V("P"); P = m.V("S")^-1 }
...
--  assert(m.match({ "S", S = m.P(msg) + m.P(1) * m.V("S") }, select(2, pcall(...))))
end
...
-- test for errors
local function checkerr (msg, ...)
--  assert(m.match({ m.P(msg) + lpeg.P(1) * m.V(1) }, select(2, pcall(...))))
end
...
assert(m.match(m.P(1) * m.B'a', 'a') == 2)
assert(not m.match(m.B(1), 'a'))
assert(m.match(m.P(1) * m.B(1), 'a') == 2)
assert(m.match(-m.B(1), 'a') == 1)

B = #letter * -m.B(letter) + -letter * m.B(letter)
x = m.Ct({ (B * m.Cp())^-1 * (m.P(1) * m.V(1) + m.P(true)) })
...
assert(m.match(m.P(1) * B, "a") == 2)
assert(not m.B(-letter):match(""))
assert((-m.B(letter)):match("") == 1)

assert((m.P(4) * m.B(letter, 4)):match("aaaaaaaa") == 5)
assert(not (m.P(4) * m.B(letter, 5)):match("aaaaaaaa"))
assert((m.P(4) * -m.B(letter, 5)):match("aaaaaaaa") == 5)

assert((m.P(3) * m.B(m.C(1))):match("12345") == 4)
...
p = m.P{
  [1] = m.P('0') * m.V(2) + m.P('1') * m.V(3) + '$',
  [2] = m.P('0') * m.V(1) + m.P('1') * m.V(4),
  [3] = m.P('0') * m.V(4) + m.P('1') * m.V(1),
  [4] = m.P('0') * m.V(3) + m.P('1') * m.V(2),
}
...
assert(m.match(m.P(1) * m.Cs(m.P(1)^0), "abcdde") == "bcdde")
...
badgrammar({[1] = -m.P("a") * m.V(1)}, "rule '1'")
badgrammar({[1] = m.P(-1) * m.V(1)}, "rule '1'")
badgrammar({[1] = m.P(1) * m.V(2), [2] = m.V(2)}, "rule '2'")
badgrammar({[1] = m.P(0), [2] = m.P(1) * m.V(1)^0}, "loop in rule '2'")
